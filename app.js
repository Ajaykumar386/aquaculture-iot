  ...
  self.update = function(after) {
    ...
    $.get(self.remoteUrl, function(data) {
      ...
      for (var z in namevaluepairs) {
        ...
        var units = "";
        if (namevalue[0].indexOf("DO") === 0) units = " mg/L";
        if (namevalue[0].indexOf("TDS") === 0) units = " ppm";
        if (namevalue[0].indexOf("Temp") === 0) units = " " + String.fromCharCode(176) + "C";
        if (namevalue[0].indexOf("Turbidity") === 0) units = " NTU";
        if (namevalue[0].indexOf("EC") === 0) units = " millisiemen/cm";
        vals.push({
          key: namevalue[0],
...
function GraphViewModel() {
  ...
  var graphLabels = ["R1", "R2", "R3", "DO", "ph", "TDS", "Temp", "Turbidity", "EC"];
  var datasets = [];
